...
class VGG16Backbone(object):
    ...
    def l2_normalize(self, x, name):
        '''
            reduce_sum, axis = -1 --> sum by row (N, W, H, C)
                        axis = 1 --> sum by row(N, C, W, H)
        '''
        with tf.name_scope(name, "l2_normalize", [x]) as name:
            ...
            x_inv_norm = tf.rsqrt(tf.maximum(square_sum, 1e-10)) #Not to do zero-division
            return tf.multiply(x, x_inv_norm, name=name)

    def forward(self, inputs, training=False): # inputs should in BGR
        feature_layers = [] # forward vgg layers
        ...
        with tf.variable_scope('conv4_3_scale') as scope:
            ...
            if self._data_format == 'channels_last':
                weight_scale = tf.reshape(weight_scale, [1, 1, 1, -1], name='reshape') #(N, W, H, C)
            else:
                weight_scale = tf.reshape(weight_scale, [1, -1, 1, 1], name='reshape') #(N, C, W, H)
...
class MobileNetV2Backbone(object):
    ...
    def forward(self, inputs, training=False):
        ...
        with tf.variable_scope('conv4_3_scale') as scope:
            ...
            feature_layers.append(tf.multiply(weight_scale, self.l2_normalize(inputs, name='norm'), name='rescale')) # conv4_3
